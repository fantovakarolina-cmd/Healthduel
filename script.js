...
window.recoverStreak = function() {
  ...
  // Seznam možných trestů / výzev pro záchranu
  const REPAIR_TASKS = [
      "udělat 50 dřepů v kuse",
      "vydržet 2 minuty v planku",
      "dát si 2 minuty ledovou sprchu",
      "udělat 30 angličáků",
...
// --- KOMPLETNÍ RESET (modal #modal v HTML) ---
// Tyto funkce dřív chyběly, takže tlačítka v okně "Opravdu reset?" nefungovala.
...
const modReset = document.getElementById('modal');
if (modReset) modReset.onclick = e => { if(e.target === e.currentTarget) window.hideReset(); };
